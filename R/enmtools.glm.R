...
#' @param step Logical determining whether to do stepwise model selection or not. This argument is deprecated and no longer does anything unless `legacy = TRUE`.
...
enmtools.glm <- function(species, env, f = NULL, test.prop = 0, eval = TRUE, nback = 1000, env.nback = 10000, report = NULL, overwrite = FALSE, rts.reps = 0, weights = "equal", bg.source = "default",  verbose = FALSE, clamp = TRUE, corner = NA, bias = NA, step = FALSE, legacy = FALSE, ...){

  if(!legacy) {
    model_args <- list(...)
    return(
      enmtools.tidy(species, env, f = f, model = "glm", test.prop = test.prop, eval = TRUE, nback = nback, env.nback = env.nback, report = report, overwrite = overwrite, rts.reps = rts.reps, bg.source = bg.source,  verbose = verbose, clamp = clamp, corner = corner, bias = bias, model_args = model_args, ...)
    )
  } else {
