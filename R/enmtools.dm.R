...
enmtools.dm <- function(species, env = NA, f = NULL, test.prop = 0, report = NULL, nback = 1000, env.nback = 10000, overwrite = FALSE, rts.reps = 0, bg.source = "default", verbose = FALSE, clamp = TRUE, corner = NA, bias = NA, legacy = FALSE, ...){

  if(!legacy) {
    return(
      enmtools.tidy(species, env, model = "dm", test.prop = test.prop, eval = TRUE, nback = nback, env.nback = env.nback, report = report, overwrite = overwrite, rts.reps = rts.reps, bg.source = bg.source,  verbose = verbose, clamp = clamp, corner = corner, bias = bias, ...)
    )
  } else {
    ...
    # dismo's domain function doesn't work with SpatRaster
