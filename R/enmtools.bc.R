...
#' @param ... Arguments to be passed to dismo::bioclim()
#'
...
enmtools.bc <- function(species, env = NA, test.prop = 0, report = NULL, overwrite = FALSE, nback = 1000, env.nback = 10000, rts.reps = 0, bg.source = "default",  verbose = FALSE, clamp = TRUE, corner = NA, bias = NA, legacy = FALSE, ...){
  ...
  if(!legacy) {
    return(
      enmtools.tidy(species, env, model = "bc", test.prop = test.prop, eval = TRUE, nback = nback, env.nback = env.nback, report = report, overwrite = overwrite, rts.reps = rts.reps, bg.source = bg.source,  verbose = verbose, clamp = clamp, corner = corner, bias = bias, ...)
    )
  } else {
...
predict.enmtools.bc <- function(object, env, maxpts = 1000, clamp = TRUE, ...){
  ...
  if(!is.na(object$species.name)){
    title <- paste("Bioclim model projection for", object$species.name)
    suit.plot <- suit.plot + ggtitle(title) + theme(plot.title = element_text(hjust = 0.5))
  }
