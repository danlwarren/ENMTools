...
check.bg.ppmlasso <- function(species, env = NA, nback = 1000, back.accurate = FALSE){
  ...
  if(!inherits(species$background.points,"data.frame")){
    ...
    if(inherits(species$range, c("raster", "RasterLayer", "RasterStack", "RasterBrick"))){
      ...
      if(back.accurate) {
        ...
        rast_poly <- rasterToPolygons(bg_rast)
        species$background.points <- as.data.frame(spsample(rast_poly, n = nback, type = "regular"))
    ...
    if(inherits(env, c("raster", "RasterLayer", "RasterStack", "RasterBrick"))){
      ...
      if(back.accurate) {
        ...
        ncells <- sum(getValues(env[[1]]) > 0, na.rm=TRUE)
        if(nback > ncells){
          agg_fact <- round(sqrt(nback / ncells))
          if(agg_fact > 1) {
